...
/*사원의 보너스 포함 연봉((급여+(급여+보너스))
연봉 = (월급* 보너스)+
연봉= (월급 * 보너스)* 12*/
-- 7.EMPLOYEE테이블에서 SAL_LEVEL이 S1인 사원의 이름, 월급, 고용일, 연락처 조회
SELECT EMP_NAME, SALARY, HIRE_DATE, PHONE
FROM EMPLOYEE
--WHERE 6000000<= SALARY AND SALARY <= 10000000;
...
--SELECT EMP_NAME, SUBSTR(SYSDATE, 1,2)+ (100-SUBSTR(EMP_NO, 1,2)), SUBSTR(SYSDATE, 3,4)
SELECT SUBSTR(EMP_NO, 1,2), SUBSTR(EMP_NO,3,2), SUBSTR(EMP_NO,3,2)
FROM EMPLOYEE;
...
SELECT EMP_NAME, FLOOR(ABS(HIRE_DATE-SYSDATE)) AS "근무일수1", FLOOR(SYSDATE-HIRE_DATE) AS "근무일수"
FROM EMPLOYEE;
...
--SELECT EMP_NAME, DEPT_CODE, TO_CHAR(SUBSTR(EMP_NO,1,6) 'YY"년"MM"월"DD"일"'),SUBSTR(SYSDATE, 1,2)+ (100-SUBSTR(EMP_NO, 1,2)) AS "나이"
SELECT EMP_NAME, DEPT_CODE, TO_CHAR(SUBSTR(EMP_NO,1,6), 'YY"년"MM"월"DD"일"'),SUBSTR(SYSDATE, 1,2)+ (100-SUBSTR(EMP_NO, 1,2)) AS "나이"
/*SELECT EMP_NAME, DEPT_CODE, SUBSTR(EMP_NO,1,2) || '년' || SUBSTR(EMP_NO,3,2) || '월' || SUBSTR(EMP_NO,4,2) || '일' AS "생년월일",
SUBSTR(SYSDATE, 1,2)+ (100-SUBSTR(EMP_NO, 1,2)) AS "나이",
EXTRACT(YEAR FROM SYSDATE) -
CASE WHEN SUBSTR(EMP_NO,1,2) >= 50 THEN 1900 + SUBSTR(EMP_NO,1,2)
ELSE 2000 + SUBSTR(EMP_NO,1,2)
END*/
FROM EMPLOYEE;
SELECT EMP_NAME, DEPT_CODE, TO_CHAR(TO_DATE(SUBSTR(EMP_NO,1,6), 'YY/MM/DD'), 'YY"년"MM"월"DD"일"'),  --주민번호에서 6자리를 빼온뒤 TO_DATE를 사용해데이터 타입으로 포멧하고
                                                                                                    --TO_CHAR를 이용해 데이터타입을 문자형식으로 변환 
                                                                                                    --그러므로 2단계를 거쳐야 한다.
EXTRACT(YEAR FROM SYSDATE) -
CASE WHEN SUBSTR(EMP_NO,1,2) >= 50 THEN 1900 + SUBSTR(EMP_NO,1,2)
ELSE 2000 + SUBSTR(EMP_NO,1,2)
...
SELECT COUNT(*) AS "전체직원수",
    COUNT(DECODE(EXTRACT(YEAR FROM HIRE_DATE), 2001,1, NULL)) AS "2001년",       --COUNT는 널을 세지 않는다.
        COUNT(DECODE(EXTRACT(YEAR FROM HIRE_DATE), 2002,1, NULL)) AS "2003년",
        COUNT(DECODE(EXTRACT(YEAR FROM HIRE_DATE), 2003,1, NULL)) AS "2004년",
