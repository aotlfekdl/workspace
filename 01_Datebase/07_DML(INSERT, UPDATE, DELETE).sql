/*
    ...
    DML(MANIPUATION 데이터 조작어) : INSERT, UPDATE, DELETE
...
SELECT * FROM EMPLOYEE WHERE EMP_NAME = '최지원';
...
--EMP_SALARY 테이블에서 노옹철사원의 급여를 100만으로 변경
--선동일 사원의 급여를 700만원, 보너스를 0.2로 변경
--EMP_SALARY 테이블에서 전체사원의 급여를 기존 급여에 10% 인상된 금액으로 변경해라
...
/*
    4. DELETE
    테이블에 기록된 데이터를 삭제하는 구분(한 행 단위로 삭제가 된다.)
