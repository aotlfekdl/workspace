/*
    <DCL : 데이터 제어문>
    계정에게 시스템권한 또는 객첼접근 권한을 부여하거나 회수하는 구문
    
    >시스템권한 : DB에 접근하는 권한, 객체를 생성할 수 있는 권한
    >객체접근권한: 특정 객체를 조작할 수 있는 권한다.
    
    CREATE USER 계정명 IDENTIFIED BY 비밀번호
    GRANT 권한(RESOUTCE, CONNECT) TO 계정;
*/
...
COMMIT;
--사번이 214, 216, 217인사람 삭제

DELETE FROM EMP_01
WHERE EMP_ID IN(802,801);
...
/*
    ACID속성 :  트랜잭션의 일관성과 신뢰성을 보장하기 위해 지켜야하는 네가지 핵심 속성
    원자성 : 트랜잭션 내의 작업이 모두 완료(COMMIT)되거나, 모두 취소(ROLLBACK)되어야함
    일관성 : 트랜잭션이 실행되기 전과 후에 데이터베이스의 상태가 항상 일관성을 유지해야함 
    독립성 : 동시에 실행되는 트랜잭션이 서로에게 영향을 미치지 않도록 돌립적으로 처리되어야한다.
    지속성 : 트랜잭션이 성공적으로 완료된 후에는 시스템 장애가 발생하더라도 영구적으로 지정되어야 함
