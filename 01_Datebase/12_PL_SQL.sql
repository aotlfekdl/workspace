/*
    ...
    -[예외처리부] : EXCEPTION으로 시작, 예의 발생시 해결하기 위한 구문
*/
SET SERVEROUTPUT ON;
...
--2 BEGIN 실행부
--<조건문>
--1) IF조건식 THEN 실행내용 END IF; (IF문 단독으로 사용할 때)
...
BEGIN
    ...
    IF BONUS = 0
        THEN DMBS_OUTPUT.PUT_LINE('보너스를 지급받지 않는 사원입니다.');
    END IF;
...
END;
/
--3) IF 조건식1 THEN 실행내용 1 ELSIF 조건식2 THEN 실행내용2...(ELSE 실행내용) END IF;
...
BEGIN
    UPDATE EMPLOYEE
    SET EMP_ID = '&변경할사번' --형변환 때문에 ''사용
    WHERE EMP_NAME = '노옹철';
EXCEPTION
    --WHEN ZERO_DIVIDE THEN DBMS_OUTPUT.PUT_LINE('나누기 연산시 0으로 나눌 수 없습니다.');
    WHEN DUP_VAL_ON_INDEX THEN DBMS_OUTPUT.PUT_LINE('이미존재하는 사원입니다.');
END;
/
