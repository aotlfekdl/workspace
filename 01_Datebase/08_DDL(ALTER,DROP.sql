...
--1_2) 컬럼 수정
--데이터타입 수정 : MODIFY 컬럼명 바꾸고자하는 데이터 타임
--DEFAULT값 수정 : MODIFY 컬럼명 DEFAULT 변경할 기본값

ALTER TABLE DEPT_TABLE MODIFY DEPT_ID CHAR(5);
ALTER TABLE DEPT_TABLE MODIFY DEPT_ID NUMBER; --형식 오류 --> 이미 들어가 있는 값이 있기 때문
...
ALTER TABLE DEPT_COPY DROP COLUMN LOCATION_ID;  --삭제하고자 할 때 컬럼이 1개만 남았다면 삭제 불가능
...
/*
    ...
    -FOREIGN KEY : ALTER TABLE 테이블명 ADD FOREIGN KEYZ(컬럼명) REFERENCES 참조할 테이블명(컬럼명);
    -UNIQUE : ALTER TABLE 테이블명 ADD UNIQUE(컬럼명);
    -CHECK : ALTER TABLE 테이블명 ADD CHECK(컬럼에 대한 조건식);
    -NOT NULL : ALTER TABLE 테이블명 ADD MODIFY 컬럼명 NOT NULL;
...
ALTER TABLE DEPT_TABLE 
        ADD CONSTRAINT DTABLE_PK PRIMARY KEY(DEPT_ID) --추가라서 ADD사용
        ADD CONSTRAINT DTABLE_UQ UNIQUE (DEPT_TITLE)  --추가라서 ADD사용
        MODIFY LNAME CONSTRAINT DTABLE_NN NOT NULL;   --수정이라서 ADD사용 안함
...
--DROP TABLE 테이블명 CASCADE CONSTRAIONT
...
--3) 컬럼명/ 제약조견명/테이블명 변경
--3_1) 컬럼명 변경: RENAME COLUMN 기존 컬럼명 새 바꿀 컬렴명
--DEPT_TITLE -> DEPT_NAME
ALTER TABLE DEPT_TABLE RENAME COLUMN DEPT_TITLE TO DEPT_NAME;

--3_2) 제약조건명변경 : RENAME CONSTRAINT 기존제약조건명 TO 바꿀제약조건명;
ALTER TABLE DEPT_TABLE RENAME CONSTRAINT SYS_C009383 TO DTABLE_ID_NN;

--3_3) 테이블명 변경: RENAME TO 바꿀테이블 명
