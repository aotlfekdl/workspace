/*
    <트리거>
    내가 지정한 테이블에 INSERT, UPDATE, DELETE등 DML문에 대한 변경사항이 생길 때 
    자동으로 매번 실행할 내용을 미리 정의해둘 수 있다.
    
    EX) 
    회원탈퇴시 기존의 회원테이블에 데이터DELETE한 후 곧바로 탈퇴한 회원들만 따로 보관하는 테이블에
    자동으로 INSERT시켜야한다.
    신고횟수가 일정 수를 넘었을 때 묵시적으로 해당 회원을 블랙리스트로 처리하게끔.
    입출고에대한 데이터 기록(INSERT)될때마다 해당 상품에 대한 재고수량을 매번 수정(UPDATE) 해야한다.

    *트리거의 종류
    -SQL문의 실행시기에 따른 분류
    -BEFORE TRIGER : 내가 지정한 테이블에 이벤트가 발생되기 전에 트리거 실행
    -AFTER TRIGER : 내가 지정한 테이블에 이벤트가 발생한 후에 드리거 실행
    
    -SQL문에 의해 영향을 받는 각 행에 딸느 분류
...
VALUES(SEQ_EID.NEXTVAL, '김지원', '11111-222222', 'D7', 'J7', SYSDATE);
...
INSERT INTO TB_PRODUCT VALUES(200, '갤럭시s25', '삼성', 1500000, DEFAULT);
INSERT INTO TB_PRODUCT VALUES(205, '아이폰15', '애플', 1400000, 10);
INSERT INTO TB_PRODUCT VALUES(210, '대륙폰', '샤오미', 800000, 20);
COMMIT;

--2. 상품 입출고 상세 이력 테이블 생성(TB_PRODETAIL)
--어던 상품이 어떤 날짜에 몇개가 입고 또는 출고가 되는지 데이터 기록
...
/*
    --상품이 입고 -> 해당상품 찾아서 재고수량 증가 UPDATE
        UPDATE TB_PRODUCT
            ...
            WHERE PCODE = 입고된 상품번호(INSERT된 자료의 DECODE);
...
INSERT INTO TB_PRODETAIL 
VALUES(SEQ_DECODE.NEXTVAL, 210, SYSDATE, 10, '입고');
...
--210번 상품이 오늘날짜로 10개 출고
INSERT INTO TB_PRODETAIL 
VALUES(SEQ_DECODE.NEXTVAL, 210, SYSDATE, 10, '출고');
