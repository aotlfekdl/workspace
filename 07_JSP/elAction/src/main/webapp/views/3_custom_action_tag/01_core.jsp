...
<body>
    ...
    *변수 삭제(c:remove var="제거하고싶은 변수명" [scope=""])
    -해당 scope영역에 해당 변수를 찾아서 제고하는 태그로
    ...
    <pre>
        ...
        향상된 for문 - (c:forEach var="변수명" items="순차적으로 접근할 객체(배열/컬렉션)" [varStatus="현재 접근된 요소의 상태값"])
    </pre>
    ...
    <%
        ...
        list.add(new Person("최지이", 19, "남자"));
        list.add(new Person("최지삼", 20, "여자"));
        %>
    ...
    <h3>6. url 쿼리스트링</h3>
    <pre>
        url경로를 생성하고, 쿼리스트링을 정의해둘 수 있으 태그
