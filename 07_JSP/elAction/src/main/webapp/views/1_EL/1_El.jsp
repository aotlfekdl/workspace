...
<body>
    <h1>1. 기본 방식대로 스크립틀릿과 표현식을 이요하여 각 scope에 데이터를 출력</h1>
    <%--
    <%
        //request scope값 가져오기
        String classRoom = (String)request.getAttribute("classRoom");
        Persion student = (String)request.getAttribute("student");

        //session scope값 가져오기
        String academy = (String)session.getAttribute("academy");
        Persion teacher = (Persion)session.getAttribute("teacher");
    %>
    ...
    --%> 
    <h3>2. EL을 이용하여 보다 쉽게 scope의 값을 출력할 수 있다.</h3>

    <p>
        EL을 이용하여 getter를 통해 가져올 필요 없이 EL구문내의 키 값만 제시하면 바로 접근 가능 <br>
    ...
    <p>EL은 공유범위가 가장 작은 scope부터 해당 키값을 검색한다(page -> request -> session -> application). </p>
