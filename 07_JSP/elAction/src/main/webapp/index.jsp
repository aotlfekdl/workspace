...
<body>
	...
	<%--
		*JSP를 이루는 구성
		1. 스크립팀 원소 : jsp페이지내에서 자바코드를 사용할 수 있는 기술
		ex) 스크립틀릿 , 표현식 ...
		
		2. 지시어 : jsp페이지 정보에 대한내용을 표현한다거나 또 달느 페이지로 포함할 떄 사용
					jsp기능을 확장시키는 라이브러리를 등록할 때 사용
					
		3. 액션태그 : jsp페이지에서 어떤 동작을 하도록 지시하는 태그
					xml기술을 이용하여 기존의 jsp문법을 확장하는 기술을 제공하는 태그 
			>>표준액션태그 : jsp페이지에서 바로 사용이 가능(별도의 연동이 필요없다.)
				-> 모든 태그명 앞에 jsp: 접두어를 붙여서 사용한다. <jsp:태그명>
			>>커스텀액션태그 : jsp페이지에서 바로 사용 불가능 별도의 라이브러리 연동이 필요)
					-> 접두어에 jsp: 이 아닌 모든 액션 태그(가장 많이 사용하는 라이브러리 jst1)
	...
	<p>
		...
		자바코드를 직접 사용하지 않고도 반복문, 조건문, 포멧처리..쉽게 수행할 수 있다. <br>
		태그로 작성이 되어있기 때문에 코드 가독성이 높아짐. <br>
	</p>
	<h3>jstl라이브러리 추가</h3>
	1) maven repository 사이트 접속 <br>
	2) jakarta standard Tag Library Tmplementation와 jakarta Standard Tag Library API 다운로드 <br>
	3) 2개 파일의 .jar 파일을 WEB-INF/lib 폴더에 추가 

	<h3>JSTL 선언 방법</h3>
    
	<p>
		JSTL을 사용하고자 하는 해당 jsp페이지 상단에 <br>
		taglib 지시어를 사용해서 선언한다. <br><br>
