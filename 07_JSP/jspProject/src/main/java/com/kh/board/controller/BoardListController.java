...
public class BoardListController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount = new BoardService().selectListCount();  //현재 총 게시글 수  
		currentPage = Integer.parseInt(request.getParameter("cpage"));   //현재 페이지(사용자가 요청한 페이지)
		
		PageInfo pi = new PageInfo(listCount, currentPage, PageLimit, boardLimit);  //하단에 보여지는 페이지 수
	
		
		ArrayList<Board> list = new BoardService().selectList(pi);  //하단에 보여지는 페이지의 배열 ex 1, 2, 3, 4, 5, 6, ...10
