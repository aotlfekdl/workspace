...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//Get방식으로 요청시 해당 doGET메서드가 저절로 호출된다.(톰켓이 서블릿 객체를 생성해서 메서드 호출까지 함
		System.out.println("GET요청 도착");
		/*
		 * 첫번쨰 매개변수인 request에는 요청시 전달된 내용들이 담겨있음(사용자가 입력한 값, 요청방식, 요청자의 url등---_
		 * 두번째 매개변수인 reponse에는 요청 처리 후 응답에 사요되는 객체(어떤 타입으로 응답을 할지, 어떤 값을 응답할지 등을 넣어주면 됨)
		 * 
		...
		//1)응답으로 출력할 내용은 html이고 무낮셋은 utf-8 선언
		...
		out.printf("<span>%d</span>님은",age);
		out.printf("<span>%s</span>님은",city);
		out.println("성별은");
		if(gender ==null) {
