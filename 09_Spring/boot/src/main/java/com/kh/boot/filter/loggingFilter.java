...
/*
    Interceptor 와 AOP는 스프링 내부에서 동작하는 반면, Filter는 스프링 외부에서 Servlet에 요청이 도착 전에 동작한다.

    로그를 띄우려면 외부에서 전달받은 Request값을 스프링으로 보내기 전에 한번은 읽어와야 하는데
    서블릿의 전첵값은 스트림으로 전달되기 때문에 딱 한번만 읽어올 수 있음
    ...
    단점
    -모든 요청에 대해서 처리하기때문에 무조건 모든 로직이 동작한다.
    - Spring MVC컨트롤러 내부 동작을 알 수 없음
 */

//OncePerRequestFilter
//일반적인 Filter는 여러번 호출될 위험이 있음
//그래서 한 요청당 필터를 한번만 실행하여 중복을 방지하는 역할이 더해진 Filter
@Slf4j
//@Component
...
////        }
////
//        //다음으로 실행될 Spring mvc 컨트롤러에서 요청 본문을 읽을 수 없ㅇ므
//
...
//              filterChain.doFilter(requestWrapper, responseWrapper);
//
//              log.info("request:{}", requestWrapper.getContentAsString(), StandardCharsets.UTF_8);
//          }finally{
