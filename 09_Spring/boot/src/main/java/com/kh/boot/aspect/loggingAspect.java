...
/*
    ...
    단점
    - AOP설정을 추가해줘야 하기 때문에 조금 복잡하다.
 */
 /*
        <시점>
        @Before : 대상메서드 실행 전에 Advice(추가 기능)가 실행된다.

        @After : 대상메서드 실행 후에 Advice(추가 기능)이 실행된다.

        @AfterReturning : 대상 메서드가 정상적으로 반환된 후에 Advice(추가기능)

        @AfterThrowing : 대상 메서드가 비정상적으로 반환(예외를 반환) 후에 Advice(추가기능)
...
public class loggingAspect {
    ...
    //com.kh.boot.RESTController 패키지의 하위 클래스의 모든 메서드에 전부 적용하겠다.
    ...
    public Object around(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        //실행해야하는 로직
        Object result = joinPoint.proceed(); //원래 하려고 햇던 작업 실행
        long endTime = System.currentTimeMillis();

        long runTime = endTime - startTime;
        log.info("--------------------------------------------------------");
        log.info("info : {} ", joinPoint.getSignature());
        log.info("execution times : {}ms ",runTime);
