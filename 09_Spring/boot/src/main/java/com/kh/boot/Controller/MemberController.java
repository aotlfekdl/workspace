...
public class MemberController {
    ...
    /*
     ...
     클래스내에서 필요한 개체를 직접 생성하지 않고 spring컨테이너가 관리하는 객체(Bean에 등록)을 주입받아서 사용할 수 있게 해줌
     필드 주입 방식/생성자주입방식
       ...
       필드 주입 방식
       스프링 컨테이너가 객체를 생성 후, @Autowired가 붙은 필드에 의존성을 주입해주는 방식
       ...
       단점 : 테스트가 어려움(필드 주입 방식은 객체 생성시 의존성이 주입되지 않고 bean에서 생성 후 주입받는 방식이기 때문에
              ...
              불변성을 보장할 수 없다. 객체 생성시 의존성이 주입되어 고정되지 않기 때문에 클래스 생성 이후에에 의존성이 변경될 수 있음

            생성자 주입 방식
            스프링 컨테이너가 객체를 생성할 때 @Autowired어노테이션이 붙은 생성자를 통해 필요한 의존성을 주입하는 방식
            장점 :  불변성 보장, 테스트가 편리하다. 순환참조 방지
...
//    }
    ...
    public String insertMember(Member member, HttpSession session, Model model) {
        /*
            age같은 int로 필드를 구성할 경우 빈 문자열이 전달되면 형변환 과정에서 400에러가 발생한다.
            보통 400에러는 보내는 데이터와 받는 데이터의 타입이 일치하지 않을 때 발생한다.

            비밀번호를 사용자 입력 그래도 저장한다...(평문)
            Bcrypt방식을 이용해서 암호화 작업 후 저장함
            -> 스프링 시큐리티에서 제공하는 모듈을 이용 (pom.xml에 라이브러리 추가후 빈에 객체 등록)
         */
        ...
        if(result > 0){
            session.setAttribute("alertMsg", "성공적으로 회원가입 완료");
            return "redirect:/";
