...
public class InformationMenu {
	...
	public void administrator() { //관리자메뉴
		while(true) {
			...
			case 5:
				this.movieReservationTable();//관리자메뉴 - 영화 상영 일정 및 좌석테이블 생성
			...
			case 7:
				this.deleteMovietable();//관리자메뉴 - 영화테이블 삭제
				break;
			case 9:
	...
	public void updateMovie() {//관리자메뉴 - 영화 정보 수정
		System.out.print("수정할 영화제목을 입력해주세요 :");
		String title = sc.next();
		...
		String director = sc.next();
		System.out.print("수정할 영화 제작사 :");
		String company = sc.next();
		
		ic.updateMovie(title, age, genre, director, company);
	}

	public void deleteMovie() {//관리자메뉴 - 영화 삭제
	...
	public void movieReservationTable(){//관리자메뉴 - 영화 상영 일정 및 좌석테이블 생성
	...
	public void selectMemberAll() {//관리자메뉴 - 회원 목록 출력
		ic.selectMemberAll();
	}
	
	public void deleteMovietable() {//관리자메뉴 - 영화테이블 삭제
		...
		ic.deleteMovietable(title, timeHH, timeMM);
...
//===============================       회원메뉴         ===================================
	public void member() {
		while(true) {
			...
			switch(choice) {
			case 1:
				this.insertMember();//회원메뉴 추가하기
				break;
				
			case 2:
				this.selectMember();//회원메뉴 정보 출력
				
				break;

			case 3:
				this.updateMember();//회원메뉴 정보 수정
				break;

			case 4:
				this.deleteMember();//회원메뉴 회원탈퇴
				
				break;

			case 5:
				ic.selectMovie();// 영화 종류 출력
				break;

			case 6:
				this.movieReservation();//회원메뉴 영화 확인
				break;
	...
	public void insertMember() {//회원메뉴 추가하기
	...
	public void selectMember() {//회원메뉴 정보 출력
	...
	public void updateMember() {//회원메뉴 정보 수정
	...
	public void deleteMember() {//회원메뉴 회원탈퇴
	...
	public void movieReservation() {//회원메뉴 영화 확인
		...
		list = ic.movieSeat(title, timeHH, timeMM);
		if(list.isEmpty()) {
			return;
		}
		...
		//ic.searchMovieTitleTime(titlename)
		ic.movieReservation(userid,title, tablename,  seatrow, seatnumber);
