...
public class Run {
	 ...
	 * 
	 * 오버라이딩 전 : Object 클래스의 toString() 실행 -> 클래스명 + @ + 객체의 주소를 암호화한 16진수값이 나온다.
	 * 오버라이딩 후 : Man클래스(자식클래스)의 toString()을 실행
	 * 
	 * *오버라이딩
	 * -자식클래스가 상속받고있는 부모 클래스의 메서드를 재정의하는 것
	 * -부모가 제공하는 메서드를 자식이 일부 고쳐서 사용하겠다는 의미
	 * 
	 * *오버라이딩의 성립 조건
	 * -부모메서드명과 메서드 명이 동일
	 * -매개변수의 갯수, 자료형, 순서도 동일해야한다.
	 * -반환형도 동일
	...
	public static void main(String[] args) {
		...
		//참조변수에 객체의 주소가 없고 null이라면null이 출력된다.
