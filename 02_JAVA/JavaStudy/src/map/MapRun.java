...
public class MapRun {
	...
	public static void main(String[] args) {
		...
		//계층구조를 보면
		//List계열 , Set계열의 클래스들은 Collection을 구현화한 클래스이다.
		//< 객체를 추가하고잘 할 때 공통적으로 add메서드를 이용
		
		//Map계열은 Collection구현한 클래스가 아니다.
		//-> 객체를 추가할 때 put메서드를 사용(key, value쌍으로 담는)
		
		//1. put(k,v) : map에 k, v 쌍으로 값을 추가하는 메소드
		
		hm.put("다이제" , new Snack("초코맛", 2000));
		hm.put("도리토스", new Snack("나초치츠맛", 500));
		hm.put("먹태깡", new Snack("와사비맛", 600));
		hm.put("틴티", new Snack("초코맛", 1000));		

		//저장되는 순서 유지가 안됨! value값이 중복되어도 키값이 중복되지 않으면 장점됨
	
		hm2.put(new Snack("초코맛", 2000), "다이제");
		hm2.put(new Snack("나초치츠맛", 500),"도리토스");
		hm2.put(new Snack("와사비맛", 600),"먹태깡");
		hm2.put(new Snack("초코맛", 1000),"틴티");		
