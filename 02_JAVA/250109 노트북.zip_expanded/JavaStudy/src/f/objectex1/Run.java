...
public class Run {
	 ...
	 * 
	 * 2. 객체란 : 현실세계에서 독립적(목적이 있고 가치가 있는 것)으로 존재한느 모든 것을 의미
	 ...
	 * 		4) 촤종적으로 추려진 속성들을 어떤 자료형, 어떤 변수명으로 사용할 것인지 정의
	 *  
	 * 		ex) 학생 관리 프로그램
	 * 		1) 학생관련 객체(김민수훈련생 ... 최지원훈련생... 김민지훈련생...)
	 * 		2) 공통적인 속성 및 기능()이름, 나이, 주소, 전화번호, 키, 몸무게, 머리색, 발사이즈, 달리기속도, 먹는다, 달린다, 잔다, 화장실간다, 공부한다, 휴식한다, 게임하다
	 * 		3) 학생 "인적사항"을 관리하는 프로그램 : 이름, 나이, 주소, 전화번호.... 
	 * 			학생 "성적"을 관리하는 프로그램 : 이름, 반, 과목별 점수, 성장률
	 * 			학생 "건강"을 관리하는 프로그램 : 이름, 반, 달리기속도, 점프력, 몸무게, 키, 혈암...
	 *		4) 이름 : String name;
	 *		   나이 : int age;
	 *			키 : double height
	 *
	 ...
	 * 			name1 : 최지원, age1 : 55
	 * 			name 2: 김지원, age2 : 60 ->name1과 age1이 하나라는 보장이 없다.
	 * 		2) "배열"로 프로그램을 만든다.
	 *			배열 : 하나의 자료형으로 여러개의 값들을 보관할 수 있음
	 *			이름을 보관하는 배열 String[] name = {"최지원", "김지원"....}
	 *			나이를 보관하는 배열 int[] age = {55, 60 ....} -> name과, gae
	 *
	 * 		3) 그래서 나온 개념 -> 구조체
	 ...
	 * 
	 * 		-> 자바에서는 구조체 개념에 좀 더 확장해서 기능(메소드)를 추가한다. = 클래스
	 * 
	 * 
	 * */
